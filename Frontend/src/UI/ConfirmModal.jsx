...
function ConfirmModal({handleSetToggleModal , warningMessage , actionName , functionToPerform}) {
    ...
    return  <Overlay>
        <div className="w-full sm:w-5/6 lg:w-2/3 h-1/2 relative">
            ...
            <div className="flex flex-col justify-center gap-14 bg-mainBlue pt-24 pb-8 px-4 sm:px-10 z-50 rounded-lg">
                ...
                <div className="flex items-center flex-col sm:flex-row justify-center gap-6 w-full">
                    <Button onClick = {handleConfirmModal} type= "confirmation">{actionName || "perform now"}</Button>
                    <Button onClick = {handleSetToggleModal} type="cancel">cancel it</Button>
                </div>
            </div>
        </div>
    </Overlay>
